...
The repository currently ships `dashforge/data/signals.yaml` with canonical signal categories and bootstrap metric
...
DashForge also has a SQLite signal store, dashboard ingestion, manual teach APIs, feedback adjustment, confidence decay,
...
DashForge should treat the packaged taxonomy as bootstrap guidance, not a rigid universal ontology. The system should
...
- `dashforge/data/signals.yaml`: provides bootstrap categories and patterns.
- `dashforge/signals.py`: persists mappings, confidence, context filters, feedback counts, review state, rejected
  candidates, and SQLite-backed learning state.
- `dashforge/dashboard_ingest.py`: infers signals from ingested dashboards and records candidates.
- `dashforge/main.py`: exposes teach, learn, approve/reject/ignore endpoints.
