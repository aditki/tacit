# ADR-001: Tacit is investigation-first, not dashboard-only
...
Tacit began as natural language to Grafana dashboards, but the repository now frames the problem as reducing
...
Tacit should be positioned as an investigation-first observability system. Grafana and SignalFx dashboards remain
...
- `tacit/pipeline.py`: records intent, path used, selected metrics, validation, dashboard URLs, and provenance.
- `tacit/history.py`: persists investigation lifecycle details.
- `tacit/models/schemas.py`: `DashResponse` remains dashboard-centered.
...
- Keep README wording honest: Tacit is demoable/experimental and not production-ready.
