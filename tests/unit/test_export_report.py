...
class FakeSignalStore:
    def stats(self):
        return {
            "signal_types": 2,
            "metric_mappings": 3,
            "ingested_dashboards": 1,
            "ingested_alerts": 1,
            "learned_artifacts": 1,
            "mappings_by_source": {"dashboard_ingest": 2, "bootstrap": 1},
            "signals_by_category": {"latency": 1, "errors": 1},
        }
...
def test_anonymous_export_writes_safe_bundle_files(tmp_path: Path, fake_stores):
    ...
    with tarfile.open(output, "r:gz") as tar:
        ...
        summary = tar.extractfile("assessment_summary.json").read().decode()
        assert "checkout-api" not in summary
...
def test_raw_export_includes_local_details(tmp_path: Path, fake_stores):
    ...
    export_assessment_report(output=output, anonymous=False)
